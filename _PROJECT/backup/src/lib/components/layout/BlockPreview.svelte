<script lang="ts">
	...
	let ref = $state<PaneAPI | undefined>(undefined);
	let iframeRef = $state<HTMLIFrameElement | null>(null);

	// Helpers / Hooks
	let isMobile = new IsMobile();
	let isMediumOrLarger = new MediaQuery("min-width: 768px");
	...
	// Derived State
	let isLargeViewport = $derived(isMediumOrLarger.current);
	...
	watch(
		() => startsInIframe,
		(shouldStartInIframe) => {
			if (shouldStartInIframe && !showIframeComp) showIframeComp = true;
		}
	);
...
<main {id} class="group">
    ...
    <!-- Component Description & Action Buttons -->
       ...
       <!-- Text Container -->
		 	<div class="relative border border-border p-4 md:p-8 m-4">
          <!-- Icon Effect -->
          <DecorIcon
