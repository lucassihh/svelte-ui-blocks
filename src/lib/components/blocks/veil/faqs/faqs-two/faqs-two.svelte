...
<section data-theme="veil" class="@container bg-background py-24">
	<div class="mx-auto max-w-3xl px-6">
		<div class="flex flex-col gap-8 @xl:flex-row @xl:items-start @xl:gap-12">
			<div class="shrink-0 @xl:sticky @xl:top-24 @xl:w-64">
			...
			<div class="flex-1">
				<Accordion type="single">
					{#each faqItems as item (item.id)}
						<AccordionItem value={item.id} class="border-dashed border-border">
							<AccordionTrigger class="cursor-pointer py-4 text-sm font-medium hover:no-underline">
								{item.question}
							</AccordionTrigger>
							<AccordionContent>
								<p class="pb-2 text-sm text-muted-foreground">{item.answer}</p>
							</AccordionContent>
						</AccordionItem>
					{/each}
				</Accordion>
