<section data-theme="veil" class="@container bg-background py-24">
	<div class="mx-auto max-w-2xl px-6">
		<h2 class="font-serif text-4xl font-medium text-balance">Create Content with AI Assistance</h2>

		<div class="mt-12 grid grid-cols-2 gap-6 @xl:gap-12">
			<p class="border-t border-border pt-6 text-muted-foreground">
				<span class="font-medium text-foreground">Generate Ideas</span> Spark creativity with AI-powered
				content suggestions and inspiration.
			</p>

			<p class="border-t border-border pt-6 text-muted-foreground">
				<span class="font-medium text-foreground">Improve Writing</span> Enhance your text with smart
				editing suggestions and style refinements.
			</p>
		</div>
	</div>
</section>
