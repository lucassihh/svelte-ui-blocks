		...
		<div class="mt-12 space-y-4">
			{#each plans as plan (plan.name)}
				<Card
					...
					class={`p-6 ${plan.highlighted ? "border border-primary" : "border border-border"}`}
				>
					<div class="flex flex-col gap-6 @lg:flex-row @lg:items-start @lg:justify-between">
						...
						<div class="w-full shrink-0 @lg:w-64">
							{#each featureKeys as featureKey (featureKey)}
								...
								<div
									class="flex items-center justify-between border-b border-border py-3 text-sm last:border-b-0"
								>
