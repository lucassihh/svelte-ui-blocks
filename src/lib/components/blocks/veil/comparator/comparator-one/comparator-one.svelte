		...
		<Card variant="outline" class="mt-12 overflow-auto *:min-w-xl">
			<div class="grid grid-cols-4 border-b border-border">
				...
				{#each plans as plan (plan.name)}
					<div
						class={`border-l border-border p-4 text-center ${plan.highlighted ? "bg-primary/5" : ""}`}
					>
			...
			{#each features as feature (feature.name)}
				<div class="grid grid-cols-4 border-b border-border last:border-b-0">
					...
					{#each planKeys as planKey, idx (planKey)}
						...
						<div
							class={`flex items-center justify-center border-l border-border p-4 text-sm ${idx === 1 ? "bg-primary/5" : ""}`}
						>
			...
			<div class="grid grid-cols-4 border-t border-border">
				<div class="p-4"></div>
				{#each plans as plan (plan.name)}
					<div class={`border-l border-border p-4 ${plan.highlighted ? "bg-primary/5" : ""}`}>
						<Button
							href="#link"
